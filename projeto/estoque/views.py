...
# def registrar_retorno_ao_estoque(request, pk):
#     entrega = ProtocoloEntrega.objects.get(pk=pk)

#     if not entrega.estoque_atualizado:
#         messages.error(request, 'Esta entrega ainda não foi baixada.')
#         return HttpResponseRedirect(reverse('produto:produto_list'))

#     # Verificar se já foi feito o registro de retorno para evitar duplicidade
#     if entrega.retorno_registrado:
#         messages.error(request, 'O retorno dos produtos desta entrega já foi registrado.')
#         return HttpResponseRedirect(reverse('produto:produto_list'))

#     # Criar um Estoque entrada
#     estoque_entrada = EstoqueEntrada.objects.create(
#         funcionario=request.user,
#         movimento='e'
#     )
#     for item in entrega.protocolo_entrega.all():
#         # Criar um Estoque Entrada Detail
#         EstoqueItens.objects.create(
#             estoque=estoque_entrada,
#             produto=item.produto,
#             quantidade=item.quantidade,
#             saldo=item.produto.estoque + item.quantidade  # Ajuste para adicionar ao estoque
#         )
#         # Atualizar o estoque do produto
#         produtos = estoque_entrada.estoques.all()
#         for item_produto in produtos:
#             produto = Produto.objects.get(pk=item_produto.produto.pk)
#             produto.estoque = item_produto.saldo
#             produto.save()

#     entrega.retorno_registrado = True
#     entrega.save()
#     messages.success(request, 'Retorno dos produtos registrado com sucesso no estoque.')
#     return HttpResponseRedirect(reverse('produto:produto_list'))